...
import { Product } from "./../../../interface/bolInterface";
import productsData from "./../../../data/bolData.json";
...
const ProductPage = () => {
	...
	useEffect(() => {
		if (id) {
			const productData = productsData.find((product: Product) => product.id === parseInt(id as string, 10));
				...
				<div className="lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-8">
						...
						<TabPanels className="aspect-h-1 aspect-w-1 w-full">
							{product.images.map((image) => (
								<TabPanel key={image.id}>
									<img alt={image.alt} src={image.src} className="h-full w-full object-cover object-center sm:rounded-lg" />
								</TabPanel>
							))}
						</TabPanels>
					</TabGroup>
					...
					<div className="mt-10 px-4 sm:mt-16 sm:px-0 lg:mt-0">
							...
							<div className="flex items-center">
								<div className="flex items-center">
									{[0, 1, 2, 3, 4].map((rating) => (
										<StarIcon key={rating} aria-hidden="true" className={classNames(product.rating > rating ? "text-indigo-500" : "text-gray-300", "h-5 w-5 flex-shrink-0")} />
									))}
								</div>
						...
						<div className="mt-6">
							...
							<div dangerouslySetInnerHTML={{ __html: product.description }} className="space-y-6 text-base text-gray-700" />
						</div>
										...
										<h3>
											<DisclosureButton className="group relative flex w-full items-center justify-between py-6 text-left">
												...
												<span className="ml-6 flex items-center">
													...
													<MinusIcon aria-hidden="true" className="hidden h-6 w-6 text-indigo-400 group-hover:text-indigo-500 group-data-[open]:block" />
												</span>
											</DisclosureButton>
										</h3>
